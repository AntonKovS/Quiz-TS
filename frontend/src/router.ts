...
export class Router {
    readonly contentElement: HTMLElement | null;        //readonly, т.к. они не перезаписываются, используются только для чтения и устанавливаются только в конструкторе.
    ...
    public async openRoute(): Promise<void> {           //Функция в любом случае возвращает промис (т.к. функция асинхронная), но тип, который будет возвращён в результате использования промиса - void, т.к. внутри функции ничего не возвращаем.
